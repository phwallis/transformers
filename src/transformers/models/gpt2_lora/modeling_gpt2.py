...
logger.info(f"{'*' * 15}RUNNING GPT2 with LoRA{'*' * 15}")
...
class GPT2Attention(nn.Module):
    ...
    def forward(
        ...
        def adapter_forward(x, weight_1, weight_2, op_name):
            scale_factor = self.adapter_alpha / self.adapter_dim
            result = x @ weight_1.type_as(x).T
            return result @ weight_2.type_as(x).T * scale_factor
